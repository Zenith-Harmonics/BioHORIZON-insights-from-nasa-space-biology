import json
from services import gpt_agent
...
class Data:
    ...
    def get_research_summarized(self, title=""):
        [osd] = self.datasets_metadata[title].keys()
        text = self.datasets_metadata[title][osd]["metadata"]["study description"]

        return self.gpt.summarize(text)
...
print(data.get_latest_research(1))
